...
const Issue = require("../models/issuesModel.jsx");
...
router.post("/issue-new-book", authMiddleware, async (req, res) => {
    try {
        //inventory adjustment(available copies must be decreased by one)
        await Book.findOneAndUpdate(
            {_id:req.body.book},
            {$inc:{availableCopies: -1}
        });
        //issue a new book to patron (create new issue record)
...
router.post("/get-issues",authMiddleware,async(req,res)=>{
    try {
        ...
        const issues = await Issue.find(req.body).populate("book").populate("user").sort({issuedDate: -1});
        return res.send({
            success:true,
